...
// Create connection
//$con = new mysqli('127.0.0.1', 'root', '', 'map', 3307); // Add the port number here
...
if (isset($_POST['email'])) {
    ...
    if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $token = bin2hex(random_bytes(20)); // Generate a token
            ...
            $sql = "UPDATE login SET token_pw = ?, token_pw_expiry = ? WHERE emailid = ?";
            ...
            if ($stmt->execute()) {
                ...
                try {
                    ...
                    $mail->Body = '<a href="http://sandboxs.online/includes/resetpassword.php?token=' . $token . '">Click here to login</a>';
                    $mail->AltBody = 'Copy and paste this URL to login: http://sandboxs.online/includes/resetpassword.php?token=' . $token;
